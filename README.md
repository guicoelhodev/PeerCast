...
The desktop host serves both the built web client and room WebSockets on port `17777`. Expose that local port through a Tailscale HTTPS proxy, then paste the resulting origin (for example, `https://host.tailnet.ts.net`) into **Share URL (Tailscale)** before creating a room. The generated host and participant links are then safe to copy to other members of the tailnet.
